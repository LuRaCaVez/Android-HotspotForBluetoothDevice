...
class BleManager(
    ...
    init {
        scope.launch {
            BleEvents.scanResults.collect { results ->
                onScanResultReceived(results)
            }
        }
    }
    ...
    private fun onScanResultReceived(results: List<ScanResult>) {
    ...
    fun startScanning() {
        ...
        bleScanner?.startScan(listOf(filter), settings, callbackIntent)
